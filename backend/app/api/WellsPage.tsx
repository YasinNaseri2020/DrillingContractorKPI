...
const WellsPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ well_id: '', name: '', pad_id: 0, status: 'online', current_depth: 0 });
    ...
    try {
      if (editingItem) {
        ...
        await fetch(API_URL, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...formData, well_id: formData.well_id }),
        });
      }
      setOpenDialog(false);
      setEditingItem(null);
      setFormData({ well_id: '', name: '', pad_id: 0, status: 'online', current_depth: 0 });
      setError('');
      loadData();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      ...
      <Button variant="contained" startIcon={<AddIcon />} onClick={() => { setEditingItem(null); setFormData({ well_id: '', name: '', pad_id: 0, status: 'online', current_depth: 0 }); setOpenDialog(true); }} sx={{ mb: 2 }}>
        Добавить скважину
      </Button>
      ...
      <Paper>
        <Table>
          ...
          <TableBody>
            ...
            {wells.length === 0 && (
              <TableRow><TableCell colSpan={8} align="center">Нет скважин. Добавьте первую.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </Paper>
      ...
      <Dialog open={deleteConfirm.open} onClose={() => setDeleteConfirm({ open: false, id: null })}>
        ...
        <DialogContent>
          <Typography>Удалить скважину? Все связанные ГТИ данные будут удалены.</Typography>
        </DialogContent>
        <DialogActions>
